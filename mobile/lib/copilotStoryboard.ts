  ...
  for (const day of dayNumbers.slice(0, MAX_DAY_SCENES)) {
    ...
    middle.push({
      ...
      scene: {
        ...
        camera: { mode: 'follow', zoom: 9.5, pitch: 55 },
        layers: {},
        narration: day === 1
  ...
  for (const camp of campPlaces) {
    ...
    middle.push({
      ...
      scene: {
        ...
        type: 'camp_arrival',
        title: camp.title,
        subtitle: camp.day ? `Overnight · Day ${camp.day}` : 'Overnight option',
        day: camp.day,
        durationMs: 5200,
        focus: { lat: camp.lat, lng: camp.lng },
        camera: { mode: 'fly', zoom: 12, pitch: 58 },
        layers: { terrain: true },
    ...
    for (const night of overnightCallouts.slice(0, MAX_CAMP_SCENES)) {
      middle.push({
        ...
        scene: {
          ...
          type: 'camp_arrival',
          title: night.title,
          subtitle: `Overnight · Day ${night.day}`,
          day: night.day,
          durationMs: 5200,
