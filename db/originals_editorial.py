...
def load_smokies_editorial_packet() -> dict[str, Any]:
    ...
    packet = _load_json(SMOKIES_EDITORIAL_PATH)
    ...
    response["artifact_sha256"] = _file_sha256(SMOKIES_EDITORIAL_PATH)
    response["summary"] = {
        "entry_count": len(packet["entries"]),
        "story_count": sum(item["kind"] == "story" for item in packet["entries"]),
        "cue_count": sum(item["kind"] == "cue" for item in packet["entries"]),
        "word_count": sum(editorial_word_count(item["transcript"]) for item in packet["entries"]),
