...
- Open P0: none. The nested sheet identity portion is device-proven. The scroll-position P1 has a focused layout-timed fix and regression test but remains open until the next paired preview proves Camp Full → campsite Full → Back to the same Camp Full scroll position. Slow-enrichment child-hub scroll proof also remains pending.
- Exact next action: publish the guarded paired preview containing `ebfe6e4`, prove its identity, and rerun only campground nested Back plus slow-enrichment Explore scroll. Continue trail/trailhead, report, and Explore-hub sheet families after that packet passes.
