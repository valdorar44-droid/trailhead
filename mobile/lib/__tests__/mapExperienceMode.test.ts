...
test('safety-sensitive map modes have deterministic priority', () => {
  assert.equal(resolveMapExperienceMode({ navigationActive: true, originalsActive: true, routeReviewActive: true }), 'navigation');
  assert.equal(resolveMapExperienceMode({ originalsActive: true, routeReviewActive: true, preview3dActive: true }), 'originals');
  assert.equal(resolveMapExperienceMode({ routeReviewActive: true, preview3dActive: true }), 'route_review');
  assert.equal(resolveMapExperienceMode({ preview3dActive: true, traceActive: true }), 'preview3d');
  assert.equal(resolveMapExperienceMode({ traceActive: true, routeBuildStatus: 'running' }), 'route_build');
});
...
test('route completion remains a durable route-review mode', () => {
  ...
  assert.equal(resolveMapExperienceMode({ routeReviewActive: true }), 'route_review');
