...
function fuelSourceLabel(estimate: FuelEstimate | null, hasRigMpg: boolean) {
  ...
  if (!estimate) return 'Using vehicle details';
  return estimate.confidence === 'estimated' ? 'Estimated' : 'Using vehicle details';
}
...
function normalizeCampSubtype(value: string) {
  ...
  if (normalized === 'informal camp') return 'Dispersed camp';
  if (normalized === 'wild camp') return 'Dispersed camp';
  if (normalized === 'rv park') return 'RV park';
  if (normalized === 'private stay') return 'Private stay';
  return String(value || '').replace(/_/g, ' ').trim();
}

function displayCampFeature(value?: string | null) {
  const clean = normalizeCampSubtype(value || '').replace(/\s+/g, ' ').trim();
  if (!clean) return '';
  const lower = clean.toLowerCase();
  if (lower === 'camp') return 'Campground';
  if (lower === 'campground') return 'Campground';
  if (lower === 'camp site') return 'Tent camp';
  if (lower === 'tent camp') return 'Tent camp';
  if (lower.includes('primitive/dispersed')) return 'Primitive/dispersed camp';
  if (lower === 'dispersed camp') return 'Dispersed camp';
  if (lower === 'rv park') return 'RV park';
  if (lower === 'rv/caravan site') return 'RV/caravan site';
...
function compactCampFeatures(camp?: Partial<CampsitePin> | null, fallback?: string, limit = 5) {
  const raw = [
    ...(camp?.site_types ?? []),
    ...(camp?.amenities ?? []),
    ...(camp?.tags ?? []),
  ...
  const seen = new Set<string>();
  return raw
    .map(item => displayCampFeature(String(item || '')))
    .filter(label => {
...
function campFilterTags(camp: CampsitePin) {
  ...
  if (camp.reservable || raw.includes('reserv')) tags.add('reservable');
  if (raw.includes('rv') || raw.includes('hookup') || raw.includes('caravan') || raw.includes('motorhome')) tags.add('rv');
  ...
  if (raw.includes('private') || raw.includes('koa') || raw.includes('resort')) tags.add('private');
  ...
  if (raw.includes('private camp')) tags.add('private_camp');
  if (raw.includes('glamping') || raw.includes('yurt') || raw.includes('cabin')) tags.add('glamping');
  return tags;
}

function campMatchesFilters(camp: CampsitePin, filters: string[], preference?: CampPreferenceMode) {
  ...
  if (preference === 'private') return isPrivate;
  if (preference === 'any' || !filters.length) return true;
...
function sourceLabel(source?: BuilderStop['source']) {
  if (source === 'camp') return 'Camp';
  if (source === 'gas') return 'Fuel';
  if (source === 'poi') return 'Place';
  if (source === 'search') return 'Selected place';
  if (source === 'map') return 'Dropped pin';
  return 'Route stop';
}
...
function RouteBuilderScreenContent() {
  ...
  function currentPlannedDaysValue() {
    return parsePositiveNumber(plannedDays) ?? parsePositiveNumber(plannedDaysRef.current);
  }

  function currentDriveHoursValue() {
    return parsePositiveNumber(driveHoursPerDay) ?? parsePositiveNumber(driveHoursPerDayRef.current);
  }
  ...
  const builderIntentFor = (inputDays: number[] = days): RouteBuilderIntent => ({
    ...
    days: inputDays,
    maxDriveHoursPerDay: currentDriveHoursValue(),
  ...
  const campDiscoveryRequestOptions = useMemo(() => ({
    limit: 220,
    mode: 'light' as const,
    stays: campTypeFilters.length === 0 || campTypeFilters.some(type => ['private', 'private_stay', 'farm', 'farm_stay', 'ranch', 'winery', 'glamping', 'private_camp'].includes(type)),
  }), [campTypeFilters]);
  ...
  const rigRouteSummary = useMemo(() => {
    if (!rigProfile || (!rigProfile.make && !rigProfile.model && !rigProfile.vehicle_type)) {
      return {
        ...
        meta: 'Uses safer fuel and drive estimates until your setup is saved.',
        ready: false,
      };
    }
    const title = [rigProfile.year, rigProfile.make, rigProfile.model].filter(Boolean).join(' ') || rigProfile.vehicle_type || 'Saved vehicle';
    const specs = [
      ...
      if (tab === 'camps') {
            ...
            if (scoped.length === 0 && campPhotoOnly && endpointCamps.length > 0) {
              scoped = endpointCamps
                ...
                .map(camp => ({
                  ...
                  route_fit: camp.route_fit || 'Area match',
                  photo_fallback_reason: 'No camp with photos was close to this overnight stop.',
                }))
                .sort((a, b) => campPreferenceScore(a) - campPreferenceScore(b) || haversineMi(a, searchLeg!.to) - haversineMi(b, searchLeg!.to))
                .slice(0, 3);
        ...
        storeDiscoveryResults(key, {
          ...
          summary: found.length
            ? builderResultSummary(found.length, 'tour', ` near this ${useLeg ? 'leg' : 'area'}`)
        ...
        storeDiscoveryResults(key, { excursions: scoped, summary: builderResultSummary(scoped.length, 'stop', ` near ${useLeg ? 'this leg' : 'this area'}`) });
      } else {
        if (useLeg) {
  ...
  function addCamp(camp: CampsitePin) {
    ...
    const campStop: BuilderStop = {
      ...
      lng: camp.lng,
      type: 'camp',
      description: camp.description || 'Camp selected for this route.',
      land_type: camp.land_type || 'camp',
      source: 'camp',
      camp,
      routeShapeRole: 'overnight',
  ...
  function addGas(station: GasStation) {
    ...
    addStop({
      ...
      lng: station.lng,
      type: 'fuel',
      description: station.address || 'Fuel stop selected for this route.',
      land_type: 'town',
      source: 'gas',
      gas: station,
    });
  ...
  function addPoi(poi: OsmPoi, routePointType: BuilderStop['routePointType'] = 'side_stop') {
    ...
    addStop({
      ...
      lng: poi.lng,
      type,
      description: `${poi.type.replace(/_/g, ' ')} stop selected for this route.`,
  ...
  function addExcursion(excursion: ExcursionCandidate, routePointType: BuilderStop['routePointType'] = 'side_stop') {
    ...
    addStop({
      ...
      type: 'waypoint',
      description: [
        excursion.summary || excursion.why_go || 'Stop selected for this route.',
  ...
  function scanBetweenStops(from: BuilderStop, to: BuilderStop, tab: DiscoveryTab, targetDay = from.day, purpose: LegSearchContext['purpose'] = 'leg', inline = true) {
    ...
    if (tab !== 'camps') setReplaceStopId(null);
  ...
  async function addDestinationFromSetup(manageLoading = true, requestId?: string) {
    ...
    try {
      ...
      const destination: BuilderStop = {
        ...
        day: Math.max(1, Math.round(currentPlannedDaysValue() ?? DEFAULT_ROUTE_DAY_COUNT)),
        name: place.name,
        lat: place.lat,
        lng: place.lng,
        type: 'waypoint',
  ...
  async function findCampAwareAnchor(day: number, count: number, spine: Array<{ lat: number; lng: number }>, totalMi: number): Promise<CampAwareAnchor> {
    ...
    let found = routeIntelligenceCamps(await api.getRouteIntelligence({
      ...
      include_stale: true,
      limit: 120,
    }).catch(() => null)).filter(camp => campMatchesFilters(camp, campTypeFilters, campPreferenceMode));
    if (found.length === 0) {
      found = uniqueByGeo((await Promise.all(
  ...
  async function findCampAwareAnchors(
    ...
    const reviewAnchors = (): CampAwareAnchor[] => windows.map(win => {
      ...
      return {
        stop: {
          id: `review_${Date.now()}_${win.day}_${Math.random().toString(36).slice(2, 6)}`,
          day: win.day,
          name: `${win.label} review area`,
  ...
  async function buildRouteFramework() {
    ...
    try {
      ...
      const plannedCount = currentPlannedDaysValue() ?? days.length ?? DEFAULT_ROUTE_DAY_COUNT;
  ...
  function buildTrip(
    ...
    const waypoints: Waypoint[] = navStops.map(st => ({
      ...
      land_type: displayCampFeature(st.land_type) || st.land_type,
      lat: st.lat,
      lng: st.lng,
    ...
    const campsites = navStops.filter(st => st.camp).map(st => ({
      ...st.camp!,
    ...
    return {
      ...
      plan: {
        ...
        route_preferences: {
          ...
          camp_reuse_policy: effectiveCampReusePolicy,
          region_hint: routeStates.join(','),
          max_daily_drive_hours: currentDriveHoursValue(),
  ...
  function renderCampPreview(stop: BuilderStop, label: string, compact = false) {
    ...
    const campFeatures = compactCampFeatures(camp, stop.land_type, compact ? 3 : 5);
    return (
      <RouteBuilderCampPreviewCard
        label={label}
        name={stop.name}
  ...
  function renderStopPreview(stop: BuilderStop, compact = false) {
    ...
    return (
      <RouteBuilderPlacePreviewCard
        ...
        color={color}
        icon={icon}
        sourceLabel={sourceLabel(stop.source)}
        typeLabel={stop.poi?.type ? displayCampFeature(stop.poi.type) : undefined}
      />
    );
  }
        ...
        {inlineTab === 'camps' ? (
          camps.length ? camps.slice(0, 6).map(camp => (
            <RouteBuilderInlineCampCard
              key={camp.id}
              title={camp.name}
              meta={`Day ${day} · ${camp.route_distance_mi != null ? `${fmtRouteDistance(camp.route_distance_mi)} off route · ` : ''}${routeProgressLabel((camp as any).route_progress) ? `${routeProgressLabel((camp as any).route_progress)} · ` : ''}${(camp as any).photo_status === 'missing' ? 'Area photo · ' : ''}${compactCampFeatures(camp, camp.land_type || 'Camp', 3)} · ${camp.cost || 'Details'}`}
              photoUrl={camp.photo_url}
        ...
        >
          ...
          {wizardStep === 0 ? (
              ...
              {(['balanced', 'direct', 'wild'] as const).map(style => (
                <TouchableOpacity key={style} style={[s.routeStyleChip, routeStyle === style && s.routeStyleChipActive]} onPress={() => setRouteStyle(style)}>
                  ...
                  <Text style={[s.routeStyleText, routeStyle === style && { color: C.orange }]}>
                    {style === 'balanced' ? 'Balanced' : style === 'direct' ? 'Direct' : 'Wild'}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            >
              ...
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text style={[s.campPreferenceTitle, campPhotoOnly && { color: C.orange }]}>With photos</Text>
                <Text style={s.campPreferenceSub} numberOfLines={1}>Only choose camps with pictures</Text>
              </View>
              <Ionicons name={campPhotoOnly ? 'checkmark-circle' : 'ellipse-outline'} size={16} color={campPhotoOnly ? C.orange : C.text3} />
            </TouchableOpacity>
            <Text style={s.setupLabel}>OVERNIGHT RHYTHM</Text>
            <View style={s.campPreferenceGrid}>
            ...
            <Text style={s.setupLabel}>BASECAMP STYLE</Text>
            <View style={s.campPreferenceGrid}>
                ...
                <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} style={s.photoGallery}>
                  {(campDetail.photos ?? []).map((uri, i) => (
                    <TouchableOpacity key={i} activeOpacity={0.9} onPress={() => setCampGalleryIndex(i)}>
                      <Image source={{ uri: campPhotoUrl(uri) }} style={[s.galleryPhoto, { width: windowWidth }]} resizeMode="cover" />
                    </TouchableOpacity>
                  ))}
                </ScrollView>
              ) : (
                ...
                <View style={s.detailTags}>
                  ...
                  {campDetail.ada && (
	                ...
	                <View style={s.detailMeta}>
	                  <Text style={s.detailCost}>{cleanCampFeeText(campDetail.price_summary?.label || campDetail.cost) || 'Details'}</Text>
