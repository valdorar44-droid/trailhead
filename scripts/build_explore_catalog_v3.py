...
def build_catalog(source_fixtures: list[str], import_out_dir: str | None = None) -> tuple[list, list, list]:
    all_records = []
    all_places = []
    all_trails = []
    fetched_at = int(time.time())
    for fixture in source_fixtures:
        records, places, trails = import_osm_fixture(fixture, fetched_at=fetched_at)
...
def main() -> int:
    ...
    parser.add_argument("--source-fixture", action="append", required=True, help="Prepared OSM-derived GeoJSON/JSON fixture.")
    ...
    records, places, trails = build_catalog(args.source_fixture, args.imports_out)
    generated_at = int(time.time())
    catalog = {
        "schema_version": 3,
        "catalog_id": "trailhead-explore-v3-real-data-foundation",
        "generated_at": generated_at,
        "source": "Prepared OSM/Geofabrik-derived fixtures; ODbL attribution preserved",
        "count": len(places),
