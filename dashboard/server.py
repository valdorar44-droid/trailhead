...
def _require_map_contributor(user: dict = Depends(_current_user)) -> dict:
    if user.get("is_admin") or has_approved_map_contributor(user.get("id")):
        return user
...
def _copilot_realtime_instructions(wake_phrase: bool) -> str:
    ...
    base = (
        ...
        "Use map_action for all map searches, place lookups, visible-area questions, map changes, selections, panels, saves, reports, downloads, route scout, route previews, or navigation confirmation. Keep spoken confirmations short. "
        ...
        "For campground searches near a named place, call map_action searchPlaces with args.category=\"camp\", args.query set to the place name, and args.open_card=true when the user asks for one campground or the best/top option. "
        "Treat map_action searchPlaces as authoritative for camp counts, pins, and opening camp cards. "
...
class DispersedLeadReviewBody(BaseModel):
    status: str
    note: Optional[str] = None
    profile: Optional[dict] = None

class DispersedLeadProfileBody(BaseModel):
    name: Optional[str] = None
    description: Optional[str] = None
    amenities: Optional[list[str]] = None
    site_types: Optional[list[str]] = None
...
class DispersedLeadPhotoBody(BaseModel):
    photo_data: str
    caption: Optional[str] = None
    content_type: str = "image/jpeg"
...
def _public_dispersed_lead(lead: dict) -> dict:
    profile = lead.get("profile_data") if isinstance(lead.get("profile_data"), dict) else {}
    return {
        "lead_key": lead.get("lead_key"),
        "id": lead.get("lead_key"),
        "lat": lead.get("lat"),
        "lng": lead.get("lng"),
        ...
        "review_flags": lead.get("review_flags") or [],
        "distance_mi": lead.get("distance_mi"),
        "reviewed_at": lead.get("reviewed_at"),
        "rejection_reason": lead.get("rejection_reason"),
        "canonical_camp_id": lead.get("canonical_camp_id"),
...
@app.get("/api/dispersed-leads/summary")
async def dispersed_leads_summary(user: dict = Depends(_require_map_contributor)):
    return {
        "access": "admin" if user.get("is_admin") else "map_contributor",
        "summary": get_dispersed_site_lead_summary(),
    }

@app.get("/api/dispersed-leads/nearby")
async def dispersed_leads_nearby(lat: float, lng: float, radius: float = 35,
                                 statuses: str = "lead,needs_field_check,community_verified,trailhead_verified",
                                 limit: int = 80,
                                 user: dict = Depends(_require_map_contributor)):
    status_list = [s.strip().lower() for s in statuses.split(",") if s.strip()]
    leads = list_dispersed_site_leads_near(lat, lng, radius_mi=radius, statuses=status_list, limit=limit)
    return {
        "access": "admin" if user.get("is_admin") else "map_contributor",
        "leads": [_public_dispersed_lead(lead) for lead in leads],
    }

@app.post("/api/dispersed-leads/{lead_key}/review")
async def dispersed_lead_review(lead_key: str, body: DispersedLeadReviewBody,
                                user: dict = Depends(_require_map_contributor)):
    status = (body.status or "").strip().lower()
    if status not in {"lead", "needs_field_check", "community_verified", "rejected", "trailhead_verified"}:
        raise HTTPException(400, "Invalid review status")
    if status == "trailhead_verified" and not user.get("is_admin"):
        raise HTTPException(403, "Admin review required")
    if body.profile:
        update_dispersed_site_lead_profile(lead_key, body.profile, reviewer_id=user.get("id"))
    lead = update_dispersed_site_lead_status(
        lead_key,
        status,
        reviewer_id=user.get("id"),
        rejection_reason=(body.note or "").strip() if status == "rejected" else None,
    )
    if not lead:
        raise HTTPException(404, "Lead not found")
    log_event(user.get("id"), None, "dispersed_lead_review", {"lead_key": lead_key, "status": status})
    return {"ok": True, "lead": _public_dispersed_lead(lead)}

@app.post("/api/dispersed-leads/{lead_key}/profile")
async def dispersed_lead_profile(lead_key: str, body: DispersedLeadProfileBody,
                                 user: dict = Depends(_require_map_contributor)):
    raw = body.dict(exclude_unset=True)
    raw.pop("note", None)
    lead = update_dispersed_site_lead_profile(lead_key, raw, reviewer_id=user.get("id"))
    if not lead:
        raise HTTPException(404, "Site not found")
    log_event(user.get("id"), None, "dispersed_lead_profile", {"lead_key": lead_key})
    return {"ok": True, "lead": _public_dispersed_lead(lead)}

@app.post("/api/dispersed-leads/{lead_key}/photos")
async def dispersed_lead_photo(lead_key: str, body: DispersedLeadPhotoBody,
                               user: dict = Depends(_require_map_contributor)):
    _decode_place_photo_payload(body.photo_data, body.content_type)
    photo = add_dispersed_site_lead_photo(
        lead_key,
        user["id"],
    ...
    if not photo:
        raise HTTPException(404, "Site not found")
    log_event(user.get("id"), None, "dispersed_lead_photo", {"lead_key": lead_key, "photo_id": photo.get("id")})
    return {"ok": True, "photo": photo}

@app.post("/api/admin/dispersed-leads/{lead_key}/publish")
async def dispersed_lead_publish(lead_key: str, body: DispersedLeadProfileBody | None = None,
                                 user: dict = Depends(_require_admin)):
    profile = body.dict(exclude_unset=True) if body else {}
    profile.pop("note", None)
    try:
        lead = publish_dispersed_site_lead(lead_key, admin_id=user.get("id"), profile_data=profile)
    except ValueError as exc:
        raise HTTPException(400, str(exc))
    if not lead:
        raise HTTPException(404, "Site not found")
    log_event(user.get("id"), None, "dispersed_lead_publish", {"lead_key": lead_key, "camp_id": lead.get("canonical_camp_id")})
    return {"ok": True, "lead": _public_dispersed_lead(lead), "camp": _camp_from_trailhead_place(lead.get("camp") or {})}
...
def _camp_source_rank(camp: dict) -> int:
    ...
    if "trailhead" in source or "trailhead" in verified:
        return 0
...
def _truthy_place_value(value: object) -> bool:
    if isinstance(value, bool):
        return value
    return str(value or "").strip().lower() in {"1", "true", "yes", "y"}

def _camp_from_trailhead_place(place: dict) -> dict | None:
    try:
        lat = float(place.get("lat"))
        lng = float(place.get("lng"))
    except Exception:
        return None
    if str(place.get("source") or "").lower() != "trailhead":
        return None
    if str(place.get("trailhead_dataset") or "") != "dispersed_camp":
        return None
    if not _truthy_place_value(place.get("trailhead_public")):
        return None
    ...
    return {
        ...
        "photos": photos,
        "reservable": False,
        "cost": place.get("cost") or "",
        "url": place.get("url") or place.get("official_url") or "",
        "official_url": place.get("official_url") or place.get("url") or "",
        "booking_url": "",
        "ada": False,
        "source": "trailhead",
        ...
        "address": place.get("address") or "",
        "provider_place_id": "",
        "place_id": str(place.get("trailhead_place_id") or place.get("id") or ""),
        "amenities": place.get("amenities") if isinstance(place.get("amenities"), list) else [],
        "site_types": place.get("site_types") if isinstance(place.get("site_types"), list) else ["Tent"],
        "activities": place.get("activities") if isinstance(place.get("activities"), list) else [],
        "access_notes": place.get("access_notes") or "",
        "bail_out_notes": place.get("bail_out_notes") or "",
        "stay_limit": place.get("stay_limit") or "",
        "reservation_notes": place.get("reservation_notes") or "",
        "max_rig_length": place.get("max_rig_length") or "",
...
def _map_card_base_from_request(body: MapCardResolveRequest) -> dict:
    ...
    return {
        ...
        "source": source,
        "source_label": body.source_label or ("Trailhead trail" if card_type == "trail" else "Place" if source == "search" else body.source or "Place"),
...
def _map_card_overnight_fallback(body: MapCardResolveRequest, card: dict) -> dict:
    ...
    fallback_source = card.get("source_label") or body.source_label or land_type
    if _map_card_bad_visible_source_label(fallback_source):
        fallback_source = land_type
    return {
        ...
        "tags": tags,
        "land_type": land_type,
        "description": card.get("summary") or card.get("address") or "Verify current access, booking rules, fees, and stay limits before relying on it.",
        ...
        "verified_source": fallback_source,
        "source_badge": fallback_source,
        ...
        "link_label": "Official page",
        "rich_detail_available": False,
        "rich_detail_reason": "Confirm details with the source before you go.",
...
_MAP_CARD_GENERIC_CAMP_WORDS = {
    "camp", "campground", "campgrounds", "camping", "campsite", "campsites",
    "site", "sites", "rv", "park", "area", "group", "tent", "shelter",
    "primitive", "dispersed", "spot", "spots", "trailhead", "trail",
}


def _map_card_name_words(value: object) -> set[str]:
    words = {
        word
        for word in re.findall(r"[a-z0-9]{3,}", str(value or "").lower())
        if word not in _MAP_CARD_GENERIC_CAMP_WORDS
    }
    return words


def _map_card_camp_name_match(body: MapCardResolveRequest, camp: dict) -> bool:
    body_token = _camp_cluster_name(body.name)
    camp_token = _camp_cluster_name(camp.get("name"))
    if body_token and camp_token and len(body_token) >= 5 and len(camp_token) >= 5:
        if body_token == camp_token or body_token in camp_token or camp_token in body_token:
            return True
    body_words = _map_card_name_words(body.name)
    camp_words = _map_card_name_words(camp.get("name"))
    if not body_words or not camp_words:
        return False
    common = body_words.intersection(camp_words)
    if len(common) >= 2:
        return True
    if len(body_words) == 1 and next(iter(body_words)) in camp_words:
        return True
    return False


def _map_card_source_text(body: MapCardResolveRequest, card: dict | None = None) -> str:
    return " ".join(str(value or "").lower() for value in (
        ...
        (card or {}).get("source"),
        (card or {}).get("source_label"),
        (card or {}).get("source_layer"),
    ))


def _map_card_rendered_source(body: MapCardResolveRequest, card: dict | None = None) -> bool:
    source_text = _map_card_source_text(body, card)
    return any(token in source_text for token in ("mapbox", "rendered", "map_tile", "map tile", "basemap", "poi-label", "place-label"))


def _map_card_camp_candidate_ok(body: MapCardResolveRequest, card: dict, camp: dict) -> bool:
    try:
        distance_m = _haversine_m(float(body.lat), float(body.lng), float(camp.get("lat")), float(camp.get("lng")))
    except Exception:
        return False
    if distance_m <= 275:
        return True
    name_match = _map_card_camp_name_match(body, camp)
    if not name_match:
        return False
    if distance_m <= (6500 if _map_card_rendered_source(body, card) else 3000):
        return True
    return False
    ...
    try:
        bridge = await discovery_context(
            DiscoveryContextRequest(
                ...
                radius=18 if _map_card_rendered_source(body, card) else 12,
                categories=["camp", "camping", "private_stay", "glamping"],
                filters=[],
        ...
        candidates = []
    if not candidates:
        candidates = await nearby_camps(body.lat, body.lng, radius=18 if _map_card_rendered_source(body, card) else 12, types="", limit=120, mode="full", stays=True)
    close = [
        camp for camp in candidates
        if isinstance(camp, dict)
        and camp.get("lat") is not None
        and camp.get("lng") is not None
        and _map_card_camp_candidate_ok(body, card, camp)
    ]
...
def _map_card_bad_visible_source_label(value: object) -> bool:
    clean = str(value or "").strip().lower()
    if not clean:
        return True
    return bool(re.search(r"\b(mapbox|rendered\s+map|map\s+feature|map\s+source|map\s+search|map\s+data|map\s+tile|basemap)\b", clean))


def _map_card_display_source_label(card: dict, body: MapCardResolveRequest, enriched_by: object = None) -> str:
    for value in (
        card.get("source_label"),
        card.get("source_badge"),
        card.get("verified_source"),
        enriched_by,
    ):
        if value and not _map_card_bad_visible_source_label(value):
            return str(value)
    display = card.get("display_type") or card.get("land_type") or card.get("subtype") or body.subtype or body.type or body.kind or "Place"
    display = re.sub(r"[_-]+", " ", str(display or "Place")).strip()
    return display[:1].upper() + display[1:] if display else "Place"
...
async def resolve_map_card(body: MapCardResolveRequest, user: dict | None = Depends(_optional_user)):
    ...
    if camp_card:
        ...
        camp_label = camp_card.get("source_badge") or camp_card.get("verified_source") or camp_card.get("source_label") or camp_card.get("source")
        if camp_label:
            card["source_label"] = camp_label
            card["source_badge"] = camp_label
        if camp_card.get("source"):
            card["source"] = camp_card.get("source")
        for key in (
    ...
    display_source_label = _map_card_display_source_label(card, body, enriched_by)
