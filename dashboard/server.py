...
def _copilot_realtime_instructions(wake_phrase: bool) -> str:
    ...
    base = (
        ...
        "For campground searches near a named place, resolve the named place first with trailhead_tool when coordinates are not already known, then call map_action searchPlaces with args.category=\"camp\", args.query set to the place name, and args.near set to the resolved lat/lng/name when available. "
        "Treat map_action searchPlaces as authoritative for opening camp cards; do not report no camps from a read-only place/geocode lookup alone. Set args.open_card=true when the user asks for one campground or the best/top option. "
