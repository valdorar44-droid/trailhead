...
class ExplorePublicPromotionRuntimeTests(unittest.TestCase):
    ...
    def test_public_browse_preserves_every_reviewed_manifest_identity(self):
        reviewed = [
            {
                "id": "place:reviewed:first",
                "category": "campground",
                "summary": {
                    "title": "Twin Campground",
                    "category": "campground",
                    "lat": 37.1,
                    "lng": -119.1,
                },
                "card": {"title": "Twin Campground"},
                "promoted_serving": True,
            },
            {
                "id": "place:reviewed:second",
                "category": "campground",
                "summary": {
                    "title": "Twin Campground",
                    "category": "campground",
        ...
        with (
            patch.object(server, "_load_explore_catalog", return_value={"places": reviewed}),
            patch.object(server, "_load_explore_guided_destinations", return_value=[]),
        ):
            result = server._explore_serving_query()
            counts = server._explore_visible_category_counts({"places": reviewed})

        self.assertEqual(
            [item["id"] for item in result["places"]],
            ["place:reviewed:first", "place:reviewed:second"],
        )
        self.assertEqual(counts["all"], 2)

    def test_dynamic_search_still_dedupes_nearby_title_collisions(self):
        reviewed = [
            {
                "id": "place:reviewed:first",
                "category": "campground",
        ...
        with (
            patch.object(server, "_load_explore_catalog", return_value={"places": reviewed}),
            patch.object(server, "_load_explore_guided_destinations", return_value=[]),
            patch.object(server, "_load_explore_legacy_search_profiles", return_value=[]),
            patch.object(server, "_explore_global_seed_search_profiles", return_value=[]),
            patch.object(server, "_pakistan_trek_explore_profiles", return_value=[]),
        ):
            result = server._explore_serving_query(q="Twin Campground")
