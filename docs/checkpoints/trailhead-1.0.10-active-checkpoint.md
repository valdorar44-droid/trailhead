...
- Open P0: none. Pending P1 assertion: force one new physical iOS Search V2 generation and confirm `Yellowstone National Park` is first with a Park label, then open it once and confirm the stable NPS/park sheet. The screen still shows the pre-deployment cached generation until the query changes.
- Exact next action: on the already-open iPhone search, change `yellowstone` to `yellowstone n` and back to `yellowstone` once, capture the refreshed list and first park sheet, then continue the remaining focused iOS Plan/Downloads/Originals and background-audio/location delta.
